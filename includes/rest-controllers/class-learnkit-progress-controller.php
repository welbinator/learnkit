...
class LearnKit_Progress_Controller extends LearnKit_Base_Controller {
	...
	public function mark_lesson_complete( $request ) {
		...
		$existing = $wpdb->get_row(
			$wpdb->prepare(
				'SELECT * FROM %i WHERE user_id = %d AND lesson_id = %d',
				$table,
				$user_id,
				$lesson_id
			)
	...
	public function get_course_progress( $request ) {
		...
		if ( ! empty( $module_ids ) ) {
			...
			if ( ! empty( $lesson_ids ) ) {
				$table        = esc_sql( $wpdb->prefix . 'learnkit_progress' );
				...
				$completed_lesson_ids = array_map(
					...
					(array) $wpdb->get_col(
						$wpdb->prepare(
							// Table name is esc_sql()'d at assignment; $placeholders expands to %d,%d,... tokens.
	...
	private function maybe_fire_course_completed( $lesson_id, $user_id ) {
		...
		$total        = count( $lessons );
		$progress_tbl = esc_sql( $wpdb->prefix . 'learnkit_progress' );
